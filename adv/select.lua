...
--	eqwait{ scenario="1" }	-- テキストの表示終了を待つ
	...
	user_sceneselect(true)	-- シーンセレクト保存
end
----------------------------------------
-- autosave
...
function select_event(flag)
	...
	if s then
		local c = #s
		for i=1, c do
			local idx = not s[i].dis and not s[i].disable and s.idx[i]
...
function select_keyup()
	...
	for i=1, max do
		...
		if not s[n].disable and not s[n].dis then
			c = n
			break
		end
	end
...
--[[
	...
	for i=1, mx do
		...
		if tn then
			local cn = s[i].dis == 1 and s[i].cond and cond(s[i].cond)
			local slnm = cn == 1 and "select_disable" or rd and gs[i] == 1 and "select_aread" or "select"	-- font名
