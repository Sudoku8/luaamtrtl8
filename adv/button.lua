		...
		if type(k) == 'string' and cm ~= 'work' then
			...
			local grph = {
				...
				objps = function(id)
					...
					lyc2{ id=(ix), file=(fl), x=(p.x), y=(p.y), clip=(p.clip) }
				end,
...
function xslider_add(name, sub)
	if get_gamemode('ui2', name) then
		...
		local add = conf[t.def] or sys[btn.name][t.def]
		add = add + sub
...
----------------------------------------
function slider_dragin()  flg.sliderdrag = true end
function slider_dragout() flg.sliderdrag = nil  end
----------------------------------------
-- 
----------------------------------------
-- ボタンステータス変更 / 全体
