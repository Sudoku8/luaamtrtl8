...
--		e:tag{"lyprop", id=(v.idx..".0"), clip=(v.clip_c)}
	...
	-- mobile
	if game.sp then
		tag{"lyprop", id=(getBtnID("bt_blog")), visible="0"}
	end
...
function blog_cache()
	...
	for i=1, m do
		...
		if v then
			...
			if v.select then
				-- 選択肢
				local sm = lang.sample and lang.sample.select or {}
				local l  = sm[ln] or get_langsystem("select") or "選択肢"
				local x  = v.select
...
function view_backlog()
	...
	local ngtag= { txkey=1 }
	local mmmx = mulpos(btnheight)
	local mmlt = mulpos(btnleft)
...
--			end
			...
			-- 縮小
			local zm = y > mmmx and math.floor(mmmx / y * 100) or 100
			tag{"lyprop", id=(id..".1"), anchorx="0", anchory="0", left=(mmlt), xscale=(zm), yscale=(zm)}
...
function log_click(e, p)
	...
	local sw = {
		...
		jump = function()
			local no = flg.blog.page + p2
			local mx = #log.stack
			if no < mx or flg.blog.scenemode then goBacklogJump(no) end
		end,
