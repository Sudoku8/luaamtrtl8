...
--		conf_user_muteloop()
	...
	-- タイトル画面
	if getTitle() then
		if game.sp then setBtnStat('bt_title', 'c') end
...
--		setBtnStat('bt_end', 'c')
		...
		tag{"lyprop", id="500.ex", visible="0"}
	end
	gscr.conf.page = p
end
----------------------------------------
function config_textdelete()
	local id = "500.z."
	for i, v in ipairs(ex.texttable) do
		ui_message(id..v)
	end
...
function config_textex()
	...
	if t0 == "on" and t1 and t2 then
		...
		if not flg.config.tx then
			...
			if type(t1) == "table" then t1 = t1[ln] end
			if type(t2) == "table" then t2 = t2[ln] end
			flg.config.tx = {
				{ s1, t1 },
				{ s2, t2 },
			}
...
function config_samplesound(e, p)
	...
	bgm = function()
		local nm = nil
		local v1 = init.config_samplebgm
