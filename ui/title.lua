...
function title_init()
	...
	if not getTrial() and not ex1 then
		-- ハッピーエンドとバッドエンドを両方見たら、メニュー画面に追加される。
		local a1 = tn(get_eval("g.b501"))	-- end1
		local a2 = tn(get_eval("g.b601"))	-- end2
		local cl = tn(get_eval("g.clear"))	-- extra open

		-- おまけ開放チェック
		local ex0 = gscr.extraopen
		if not ex0 and cl == 1 then
...
--	elseif gscr.epilogue then	route = "title02"	-- エピローグ開放
	...
	-- bg
	local ac = tn(get_eval("g.allcflag"))	-- ac
	if ac == 1 then
		local v  = getBtnInfo("bg")
		lyc2{ id=(v.idx), file=(get_uipath().."title/bg2")}
	end
...
--	if sv.checkopen("title") then setBtnStat('bt_load' , 'c') end	-- 最新のセーブデータ及びオートモード
	...
	local sw = {
		...
		title01 = function()
			...
			if not gscr.clear	  then setBtnStat('bt_true' , 'c') end
		end,

		-- 体験版
		trial = function()
			setBtnStat('bt_true' , 'c')
			setBtnStat('bt_extra', 'c')
		end,
	}
	title_page()
...
function title_anime(no)
	if no ~= "trial" then
		local tb = { "bt_start", "bt_cont", "bt_load", "bt_conf", "bt_extra", "bt_exit" }
		local x  = mulpos(-50)
		local tm = 400
		local dl = 0
