...
--		local vv = getBtnInfo('obj'..string.format("%02d", i))
		...
		-- no
		local sn = string.format("Clip.%03d", no)
		...
		if t then
			local tx   = t.text
			local nx   = tx.name or {}
			local time = "："..(nx[2] or nx[1])		-- get_osdate("%Y/%m/%d %H:%M", t.date)
...
--[[
			...
			tag{"lyprop", id=(id..'.0'), clip=(v.clip)}

		-- ない
		else
			tag{"lyprop", id=(id..'.0'), clip=(v.clip_c)}
	...
	-- active
	local bt = btn.cursor
	if bt and bt:find("_save") and game.cs then btn_active2(bt) end
...
--			if checkBtnExist('bt_save')  then setBtnStat('bt_save' , 'd') end
			if checkBtnExist('bt_save')  then tag{"lyprop", id=(getBtnID("bt_save")), visible="0"} end
		elseif game.cs then
			if checkBtnExist('bt_save') then
				setBtnStat('bt_save', 'c')
				tag{"lyprop", id=(getBtnID("bt_save")), visible="0"}
			end
			if checkBtnExist('save') then
				local v  = getBtnInfo("save")
