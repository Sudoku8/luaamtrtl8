...
function mw_textloop(p, flag)
	local v = getTextBlockFloat(p.text, nil, p.lang)
	if v then
		----------------------------------------
		-- 既読判定
		local m  = getGameMode()
...
function getNameChar(p, flag)
	local ln = get_language(true)
	local r  = nil
	if p and p[ln] and p[ln][1] and p[ln][1].name then
		r = flag and p[ln][1].name[2] or p[ln][1].name[1]
	end
	return r
end
----------------------------------------
...
function getTextBlockFloat(p, mode, name)
	local r  = nil
	local z  = p.float or {}
	local ln = name or get_language(true)
	for i, v in ipairs(p[ln]) do
