...
function exf.bginit()
	...
	for i, v in pairs(csv.extra_bgm) do
		...
		if no and no > 0 then
			...
			appex.bgmd[no] = { file=(i), no=(no), text=(v[3]), flag=(f), time=(v.time) }
		end
	end
...
----------------------------------------
-- text
function exbgm_text(nm, num)
	local tbl = {
		exbgm01 = '500.tx.20',
		exbgm02 = '500.tx.21',
	}
	local tx = num == 0 and "0:00" or math.floor(num / 60000)..':'..string.format("%02d", ((num/1000) % 60))
	ui_message(tbl[nm], { nm, text=(tx) })
end
----------------------------------------
function exbgm_timer(no)
	exbgm_text('exbgm02', no * 1000)
	flip()
end
----------------------------------------
-- 現在のページ
