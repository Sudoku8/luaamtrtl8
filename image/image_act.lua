...
function image_act(id, p)
	if id and conf.effect == 1 and not getSkip() then
		...
		local sync= (not lp or lp > 0) and p.sync == 1
		...
		local sw = {
			...
			-- ジャンプ
			["ジャンプ"] = function(p)
				local y = mulpos(p.size or 30)
				local l = image_actloop(p, "harf")
				local t = p.time or 150
				local n = p.ease or "out"
				tween{ id=(idx), y=("0,"..-y), yoyo=(l), time=(t), ease=(n)}
				if sync then eqwait(t*2*l) end
			end,
			...
			["ふるふる"] = function(p)
				local sz = mulpos(p.size or 20)
				local tm = p.time or 200
				tag{"tweenset"}
				tween{ id=(idx), x=("0,"..-sz)	 , time=(tm), ease="inout"}
				tween{ id=(idx), x=(-sz..","..sz), time=(tm), ease="inout"}
				tween{ id=(idx), x=(sz..","..-sz), time=(tm), ease="inout"}
				tween{ id=(idx), x=(-sz..","..sz), time=(tm), ease="inout"}
				tween{ id=(idx), x=(sz..",0")	 , time=(tm), ease="inout"}
				tag{"/tweenset"}
			end,
			...
			["うん"] = function(p)
				...
				local t = p.time or 600
				local n = p.ease or "out"
				tween{ id=(idx), y=("0,"..y..',0'), time=(t..','..t..','..t), ease=(n)}
			end,
...
function quake(p)
	...
	-- 
		...
		local q = init.quake
		local size = mulpos(p.size or q and q[1] or 4)	-- 揺れサイズ
		...
		elseif not getSkip(true) then
			...
			elseif dir == 'h' then
				...
				local sz = p.size or 25
				local ct = p.loop or 2
				if not p.loop and tm > 100 then
					ct = math.floor(tm / 100 / 2) * 2
					tm = 100
				end
				tween{ id=(id), y=("0,"..sz), time=(tm), yoyo=(ct), ease=(ease)}
--				tween{ id=(id), y=(rx), time=(math.ceil(time/2)..','..time), ease=(ease)}
			...
			-- sync
			if p.sync == 1 then eqwait(time*cnt*2) end
		end
	end
end
----------------------------------------
