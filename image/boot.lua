...
--			estag{"video", file=(px), skip="2"}
			estag{"brandlogo_movie", { file=(px), id=(id), path=(path), back=(p.back) }}
...
----------------------------------------
-- 動画再生
function brandlogo_movie(p)
	if p.back then lyc2{ id=(p.id), file=(p.path..p.back) } end
	movie_playfile(p.file)
	flip()
end
----------------------------------------
