...
function readImage(id, p, tone)
	...
	--------------------------------
	-- color
	if p.color and not p.style then
		local g  = game
	...
	elseif e:isFileExists(px..'.ipt') then
		...
		cut = function()
			for i, v in ipairs(ipt) do
				lyc2{ id=(idx.."."..v.id), file=(path..v.file), x=(v.x), y=(v.y), layermode=(style) }
			end
		end,
...
--			tag{"lyprop", id=(idx), left=(v.ax), top=(v.ay)}
	...
	-- 画像直
		...
		lyc2{ id=(idx..'.0'), file=(px), clip=(cl), layermode=(style) }
...
function image_loop(flag)
	...
	local settime = function(p, def, f)
		local tm = tn(p.time) or f and (p.mx or p.my) and init.fg_fade2 or p.flip and 0 or init[def] or def or 0
		local tx = tr.time
