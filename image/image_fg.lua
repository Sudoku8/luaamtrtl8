...
function fgf(p)
	...
	if t then
		...
		if p.mx or p.my then
			...
			if p.mx then tween{id=(idm), x=(mulpos(p.mx)..',0'), time=(time) } end
			if p.my then tween{id=(idm), y=(mulpos(p.my)..',0'), time=(time) } end
		elseif v then
			local time = getTweenTime(p.time, init.fg_fade)
...
function fgfdel(p)
	...
	if s[nm] then
		...
		if v then
			local x = p.mx or v.x
			local y = p.my or v.y
			if x	then tween{id=(idm), x=('0,'..mulpos(x)), time=(time) } end
			if y	then tween{id=(idm), y=('0,'..mulpos(y)), time=(time) } end
